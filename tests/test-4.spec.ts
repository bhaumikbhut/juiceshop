...
test.beforeEach("user login", async ({ page }) => {
  ...
  const repeatPasswordField = page
    .locator("mat-form-field")
    .locator("[id=repeatPasswordControl]");
...
test("should successfully add products to cart, modify cart, and complete checkout process", async ({
  page,
}) => {
  await page
    .locator("mat-card")
  ...
  await page
    .locator("mat-card")
    .filter({ hasText: "Apple Pomace 0.89¤Add to" })
    .getByLabel("Add to Basket")
    .click();
  await page.getByRole("button", { name: "X", exact: true }).click();
  await page
    .locator("mat-card")
    .filter({ hasText: "Banana Juice (1000ml) 1.99¤" })
    .getByLabel("Add to Basket")
    .click();
  await page.getByRole("button", { name: "X", exact: true }).click();
  await page
    .locator("mat-card")
    .filter({ hasText: "DSOMM & Juice Shop User Day" })
    .getByLabel("Add to Basket")
    .click();
  await page.getByRole("button", { name: "X", exact: true }).click();
  await page
    .locator("mat-card")
    .filter({ hasText: "Carrot Juice (1000ml) 2.99¤" })
    .getByLabel("Add to Basket")
    .click();
  await page.getByRole("button", { name: "X", exact: true }).click();
  ...
  await page
    .getByRole("row", { name: "Apple Juice (1000ml) Apple" })
    .getByRole("button")
    .nth(1)
    .click();
